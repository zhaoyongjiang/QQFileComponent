...
import cn.xxt.file.util.OtherScannerUtil;
...
public class OtherFragment extends FileBaseFragment {
    @Inject
    @ActivityContext
    Context context;

    @BindView(R2.id.rlv_other)
    ...
    public int getLayoutResource() {
        return R.layout.fragment_other_file;
    }
...
//        }
    ...
    private void getDataOfPhone() {
        new OtherScannerUtil(this, FileTypeEnum.TYPE_OTHER.getFileType()).getOtherFromLocal(new OtherScannerUtil.OtherFileScannerCallBack() {
            @Override
...
//                    FileDb.getInstance(context).insertFileInfoList(0, fileInfos);
    ...
    private void getDataOfDb() {
        //异步获取
        fragmentPresenter.getFileInfoListFromLocalDb(context, getWebId(), FileTypeEnum.TYPE_OTHER.getFileType());
    }
    ...
    private void handleFiles(List<FileInfo> fileInfoList) {
        try {
            ...
            FolderItem txtItem = new FolderItem("电子书");
            FolderItem zipItem = new FolderItem("压缩文件");
            FolderItem otherItem = new FolderItem("其他");

            for (FileInfo fileInfo : fileInfoList) {
                if (FileUtil.checkSuffix(fileInfo.getFileLocalPath(), new String[]{"txt"})) {
                    fileInfo.fileType = FileTypeEnum.TYPE_OTHER_TXT.getFileType();
                    txtItem.addSubItem(fileInfo);
                    ...
                    otherItem.addSubItem(fileInfo);
                }
            }

            if (txtItem.getSubItems() != null && txtItem.getSubItems().size() > 0) {
                fileTypeList.add(txtItem);
            }
            if (zipItem.getSubItems() != null && zipItem.getSubItems().size() > 0) {
                fileTypeList.add(zipItem);
            }
            if (otherItem.getSubItems() != null && otherItem.getSubItems().size() > 0) {
                fileTypeList.add(otherItem);
            }
