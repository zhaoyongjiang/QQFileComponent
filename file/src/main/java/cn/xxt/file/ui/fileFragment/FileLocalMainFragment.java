...
public class FileLocalMainFragment extends FileBaseFragment {
    ...
    public int getLayoutResource() {
        return R.layout.fragment_file_local_main_file;
    }
    ...
    public void initView() {
        ...
        if (fileTypeList.contains(TYPE_DOC.getFileType())) {
            ...
            docFragment.setDataFlag(FileBaseFragment.FLAG_DATA_LOCAL);
        ...
        if (fileTypeList.contains(TYPE_AUDIO.getFileType())) {
            ...
            audioFragment.setDataFlag(FileBaseFragment.FLAG_DATA_LOCAL);
        ...
        if (fileTypeList.contains(TYPE_IMAGE.getFileType())) {
            ...
            photoFragment.setDataFlag(FileBaseFragment.FLAG_DATA_LOCAL);
        ...
        if (fileTypeList.contains(TYPE_OTHER.getFileType())) {
            ...
            otherFragment.setDataFlag(FileBaseFragment.FLAG_DATA_LOCAL);
        ...
        String[] titles = fileTypeFragmentTitles.toArray(new String[fileTypeFragmentTitles.size()]);
//        slidingTabLayout.setViewPager(viewPager, titles, getActivity(), fragments);
        ...
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                //切换fragment，不管是使用滑动切换，还是tab切换。该处都会运行一次。。所以，通知各fragment变化，就在这。
                //如果是刚运行起来，切换fragment，这时候fragment还没有创建，所以，notify会导致空指针，所以，实现管理-编辑功能，两步走：第一，点击了编辑，setter赋值给相应的adapter。第二：切换时候，在该处nofity
    ...
    /**
     * 将管理器删除文件点击事件 转为 删除标识，分发到各个文件framgent中。待fragment显示的时候，重载数据
     * @param fileInfoList
     */
