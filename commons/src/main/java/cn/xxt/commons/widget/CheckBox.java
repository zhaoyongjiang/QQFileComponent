...
import cn.xxt.commons.util.CheckBoxUtil;

/**
 * Created by zhq on 17/10/25.
 */
...
public class CheckBox extends View {
    ...
    public CheckBox(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        size = a.getDimensionPixelSize(R.styleable.CheckBox_checkbox_size, CheckBoxUtil.dp(22));
    ...
    private void init() {
        if (paint == null) {
            ...
            textPaint.setTextSize(CheckBoxUtil.dp(16));
            ...
            eraser2.setStrokeWidth(CheckBoxUtil.dp(28));
            ...
            backgroundPaint.setStrokeWidth(CheckBoxUtil.dp(2));
        }
        backgroundPaint.setColor(borderColor);
        setCheckOffset(CheckBoxUtil.dp(1));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawBackground || progress != 0) {
            ...
            if (roundProgressCheckState < progressBounceDiff) {
                drawRad -= CheckBoxUtil.dp(2) * roundProgressCheckState / progressBounceDiff;
            } else if (roundProgressCheckState < progressBounceDiff * 2) {
                drawRad -= CheckBoxUtil.dp(2) - CheckBoxUtil.dp(2)
            ...
            if (drawBackground) {
                ...
                canvas.drawCircle(paddingLeft + rad, paddingTop + rad, drawRad
                        - CheckBoxUtil.dp(1), paint);
                canvas.drawCircle(paddingLeft + rad, paddingTop + rad, drawRad
                        - CheckBoxUtil.dp(1), backgroundPaint);
            }
            paint.setColor(isEnabled() ? normalColor : disabledColor);
            backgroundPaint.setColor(borderColor);
            bitmapCanvas.drawCircle(rad, rad, showBorderInCheckState ? rad - CheckBoxUtil.dp(1.2f) : rad,
                    paint);
            bitmapCanvas.drawCircle(rad, rad,
                    showBorderInCheckState
                            ? (rad - CheckBoxUtil.dp(1.2f)) * (1 - roundProgress)
                            : rad * (1 - roundProgress),
                    eraser);
        ...
        if (!TextUtils.isEmpty(text)) {
            textLayout =
                    new StaticLayout(text, textPaint,
                            CheckBoxUtil.dp(100), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
